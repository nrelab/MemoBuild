...
impl ClusterServer {
    pub async fn start(self, port: u16) -> Result<()> {
        ...
        let app = Router::new()
            ...
            .route("/cluster/nodes", post(add_cluster_node))
            .route("/cluster/nodes/:node_id", delete(remove_cluster_node))
            // Auto-scaling endpoints
            .route("/scaling/status", get(get_scaling_status))
            .route("/scaling/metrics", post(record_scaling_metrics))
            .route("/scaling/predict", get(predict_resources))
            // Health check
...
async fn add_cluster_node(
    ...
    let node = crate::cache::cluster::ClusterNode {
        id: req.id,
        address: req.address,
    ...
    match state.cluster.add_node(node).await {
        Ok(_) => Ok(Json(serde_json::json!({ "status": "node_added" }))),
        Err(_) => Err(StatusCode::INTERNAL_SERVER_ERROR),
    }
}
...
async fn remove_cluster_node(
    ...
    match state.cluster.remove_node(&node_id).await {
        Ok(_) => Ok(Json(serde_json::json!({ "status": "node_removed" }))),
        Err(_) => Err(StatusCode::INTERNAL_SERVER_ERROR),
    }
}
...
async fn get_scaling_status(
    State(state): State<Arc<ClusterAppState>>,
) -> Result<Json<crate::network::scaling::ScalingStatus>, StatusCode> {
...
async fn record_scaling_metrics(
    ...
    match state.auto_scaler.record_metrics(metrics).await {
        Ok(_) => Ok(Json(serde_json::json!({ "status": "metrics_recorded" }))),
        Err(_) => Err(StatusCode::INTERNAL_SERVER_ERROR),
    }
}
...
) -> Result<Json<crate::network::scaling::ResourcePrediction>, StatusCode> {
